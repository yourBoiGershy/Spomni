...
# (skills, agents, scripts, templates). When the MAIN conversation (empty
# agent type) tries to Edit/Write under a protected prefix, the write is
# blocked with exit 2. docs/, data/, root markdown, and the rest of .claude/
# (rules, context, settings) stay orchestrator-editable for planning work.
# Fails OPEN on parse errors. Bash 3.2 portable.

# Space-separated, repo-relative, trailing slash. Override via env if needed.
PROTECTED_PREFIXES="${HARNESS_PROTECTED_PREFIXES:-.claude/skills/ .claude/agents/ .claude/scripts/ templates/}"

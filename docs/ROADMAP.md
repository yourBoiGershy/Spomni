...
| 40 | Dynamic sync routing — `lanes.tsv` commands use `{{REPO_ROOT}}`/`{{DATA_DIR}}`/`{{PRIVATE_DATA_ROOT}}`/`{{STORE_DIR}}`/`{{CLAUDE_BIN}}` resolved by the scheduler at every tick (sync-lanes 1.1.0); `sync-scheduler.sh init` + `resolve <lane>`; `install` retires legacy `com.relationship-agent.sync.*` agents; beeper `store_dir` optional (follows `data/store`) | connectors/scripts + core (sync-lanes, template) + connectors/beeper-in + docs/SETUP | 19, 28 | In progress (2026-08-30, worktree `chunk-40-dynamic-sync-routing`, plan `docs/plans/2026-08-30-40-dynamic-sync-routing.md`; trigger: live 15-min beeper agent found bound to a dead worktree writing to an orphan store) |

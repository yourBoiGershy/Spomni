...
| 33 | Nudge delivery — Beeper note-to-self default (`connectors/beeper-out`, self-only), gmail-self fallback, outbox audit; `profile.md` 1.1.0 `## Notify`; core `nudge-card` contract + renderer with numbered cards + reply footer; `notify` lane on the sync scheduler (deliver-on-tick, quiet hours); sweep step 8 wired | connectors (beeper-out, gmail-out, file-out, notify lane) + core (nudge-card, profile 1.1.0) + attention (sweep step 8) | 06, 28; absorbs plan 07's nudge/adapter units | Ready (2026-08-30, plan 33; research `docs/research/2026-08-30-nudge-delivery-and-feedback-loop.md`; decision notify-self-is-a-send) |
| 34 | Feedback ledger & active iteration — `feedback-event@1` + append-only `signals/feedback.jsonl` (ingestion `feedback-file.sh`, called by every existing feedback writer); numbered-reply parsing on every sync tick; corrections → `## Recent corrections` few-shot in judgment prompts + auto eval cases; phase 2 (≥2 weeks live data): kind outcome weights read before enumeration, corrections → user-model revision proposals, weekly assistant report card | ingestion (ledger, feedback-parse, judge prompt) + core (contract, ranking-weights 1.2.0) + attention (calibrate, tier-drift prefilter, weekly-planning) | 31, 33 | Ready (2026-08-30, plan 34; phase 2 gated on live ledger data) |
| 35 | Store currency, dedup, remainder speed, preference loop — person.md 1.4.0 as-of/resolved open threads + latest-interaction-wins rule + `refresh-person.sh`; `person-merge.sh` + deterministic merge-candidate detection in onboarding; noise-sender triage + one-call email filing (remainder 7.8 → <2 min); `learn-sweep.sh` turning feedback-ledger corrections into user-model rules + learned eval cases | core (person 1.4.0, person-merge) + ingestion (currency spec, thread-summary 1.1.0, noise triage, refresh-person) + attention (learn-sweep) | 31, 32, 34 phase 1 | Proposed (2026-08-30, plan 35) |

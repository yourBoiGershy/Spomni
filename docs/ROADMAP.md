...
Chunk 08 (DECISIONS.md: composio-hub) supersedes the *access mechanism* of plans 02
and 04 — pulls now go through the user's Composio account instead of first-party MCP.
...
|---|---|---|---|
| `ingestion/` | `stream-ingestion` | connectors-in + ingestion (data imports via Composio hub: gmail, calendar, linkedin, user inputs; texts = later local lane) | 08 → 02 → 04 → 03 |

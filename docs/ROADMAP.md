...
| 44 | Reminders that fire + pluggable notification service — `attention-fire` lane (hourly deterministic `fire` + `acted-on`), `attention-sweep` lane (daily headless sweep, opt-in), claude-bin resolution for launchd, jq fallback in `wakeup-queue.sh`; wave B: `notify-channel.md` registry (`channel.tsv`, first-party + user plugins), generic `deliver-tick.sh` dispatch, `calendar-out` (`calendar-self` channel, first-party Google Calendar connector, self-only), `/who-next --holds`. Mission test: *remembering-to* — a due reminder reaches the user where they look, without a session. | 06, 28, 33, 40 | Wave A **Built** 2026-09-02 (branch `chunk-44-attention-lanes-claude-bin`, plan `docs/plans/2026-09-02-44-notification-service-pluggable-channels.md`). Wave B: Ready, pending D5 calendar defaults confirmation. Owed: TCC grant / move out of `~/Documents` on the live Mac; rebuild `debrief-filed.log` before enabling `attention-sweep` |

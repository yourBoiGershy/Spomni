...
| 33 | Nudge delivery — Beeper note-to-self default (`connectors/beeper-out`, self-only), gmail-self fallback, outbox audit; `profile.md` 1.1.0 `## Notify`; core `nudge-card` contract + renderer with numbered cards + reply footer; `notify` lane on the sync scheduler (deliver-on-tick, quiet hours); sweep step 8 wired | connectors (beeper-out, gmail-out, file-out, notify lane) + core (nudge-card, profile 1.1.0) + attention (sweep step 8) | 06, 28; absorbs plan 07's nudge/adapter units | In progress (2026-08-30, `chunk-33-nudge-delivery`; research `docs/research/2026-08-30-nudge-delivery-and-feedback-loop.md`; decision notify-self-is-a-send) |

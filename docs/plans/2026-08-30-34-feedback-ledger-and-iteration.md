...
| U8b [worker] | ingestion+connectors | `<n> draft [free text]` verb (plan 33 D3/D4 amendment 2026-08-30, nudge-first/draft-on-demand): `feedback-parse.sh` resolves `n` against the last delivered batch, ledgers `--type draft-request` (free text verbatim), and writes the entry's `draft` (composed with `profile.md ## Style notes` + the free text, per U16) to `outbox/<batch>-<n>-draft.txt`; `deliver-tick.sh` posts it to the same self-chat headed `Draft (unsent):`. Entry with no `draft` field and no composable context → posts `no draft available for <n>`; never invents ammunition. Human still sends. | U8, 33 |
